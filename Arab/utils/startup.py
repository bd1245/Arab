...
LOGS = logging.getLogger("تليثون ويلسون \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/646133a60410bf55672ca.mp4", caption="🝳 ⦙ تـمّ  اعـادة تشـغيل\n تليثـون ويلسون ✓  :  [ 7.7 ] .\n\n🝳 ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n🝳 ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n🝳 ⦙ القناة الرسمية تليثون العرب : @kyany_el5as\n🝳 ⦙ فارات سورس تليثون  :@kyany_el5as \n🝳 ⦙ كلايش تليثون :  @kyany_el5as\n 🝳 ⦙التحديثات والاضافات :  @kyany_el5as\n",                buttons=[(Button.url("مطور تليثون الرسمي", "https://t.me/WELS0N"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون ويلسون العام", iqthon, Config.TG_BOT_USERNAME, descript  ,  iqphoto1 )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون ويلسون الخاص", iqthon, Config.TG_BOT_USERNAME, descript    , iqphoto2  )
